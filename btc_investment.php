    ...
    if($check_debt){
        ...
        if($num_of_debtors == 0){ 
            ...
            if($total_past_pledge == 0){
                ...
                $insert = mysqli_query($con, "INSERT INTO tp_pledges(tp_user_unique_id, tp_amount_cedis,tp_amount_coin, tp_username,tp_mobile_money_name, tp_user_mobile_number, tp_user_contact, tp_transaction_checked,tp_package,tp_payment_method)
                            VALUES(' $tp_user_unique_id','$amount_cedis','$amount_coin','$username','$mobile_money_name','$mobile_number','$contact',1,'$package','bitcoin')");
                            if($insert){
                              
                                echo '<div class="alert alert-success alert-dismissible"><span class=""></span> An Order of '.$amount_cedis.' dollars received! click on the button below to make payments</div>';
                               
                                echo '<center><form method="post" action=""><button type="submit" name="submit-paid" class="btn btn-success">CLICK TO PROCEED TO MAKE PAYMENT</button>
      ...
      elseif($package == 'premium') {
          ...
          if($amount_cedis >= 120 && $amount_cedis <= 1000 ){                    
           // echo 'yes';
...
if($num_of_users == 1){
    ...
    if($check_debt){
     ...
     elseif($package == 'client') {
          ...
          if($amount_cedis >= 1100 && $amount_cedis <= 2000 ){                    
       ...
       ?>
                     ...
                     <center>  <form method="post" action="#">
                           ...
                           <select  style="background:#d9edf;" class="form-control form-control-lg" name = "packages">
                             <option value="startup">Startup Package - $25 - $100 </option>
                             <option value="premium">Premium Package - $120 - $1000 </option>
                             <option value="client" disabled>Client Package - GHC1600 - GHC5000 </option>
                          </select>
                           
                             <center><h3>ENTER INVESTMENT AMOUNT IN DOLLARS(USD)</h3></center>
